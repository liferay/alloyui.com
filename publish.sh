...
npm install . && docpad generate && git checkout gh-pages && git rm -rf . && mv out/* . && rm -rf out && git add . && git commit -m "Regenerate" && git push origin gh-pages && git checkout master